...
export function useSmsStatus() {
  return useQuery({ queryKey: ["sms-status"], queryFn: api.smsStatus, staleTime: 15_000 });
}

export function useSmsLanguages() {
  return useQuery({ queryKey: ["sms-languages"], queryFn: api.smsLanguages, staleTime: Infinity });
}
