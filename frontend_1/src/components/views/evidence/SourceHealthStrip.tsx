      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {ADAPTERS.map((source) => {
          ...
          return (
            <div
              key={source}
              className="flex flex-col gap-1.5 rounded-xl border border-hairline bg-surface p-3"
            >
              <div className="flex flex-wrap items-center justify-between gap-1.5">
                <span className="flex min-w-0 items-center gap-1.5 whitespace-nowrap text-[11px] font-bold uppercase text-body">
                  <span className="h-2 w-2 shrink-0 rounded-full" style={{ background: SOURCE_COLORS[source] }} />
                  {SOURCE_LABELS[source]}
                </span>
                <Badge tone={present ? "good" : "neutral"} className="shrink-0">
                  {present ? "Live" : "Absent"}
                </Badge>
              </div>
