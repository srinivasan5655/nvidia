...
export function SituationEvidenceView({
  ...
  const alertRing = useMemo<[number, number][] | null>(() => deriveAlertRing(event?.items), [event]);

  if (!event) {
    return (
      <EmptyState
        title="No event loaded"
        body='Go to Home and press "Check Now" to assemble evidence from every configured source.'
      />
    );
  }
