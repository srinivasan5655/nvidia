  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      for (const item of items) {
        if (item.source === "fema" || item.source === "population_svi") continue;
        const el = document.createElement("div");
      ...
      // FEMA declarations are one county-level point (see fema.py), never a
      // precise boundary — a diamond outline (not a filled dot) plus explicit
      // popup wording keeps that honest rather than implying a precise fix.
      for (const item of items.filter((i) => i.source === "fema")) {
        ...
        const marker = new maplibregl.Marker({ element: el })
          ...
          .setPopup(
            new maplibregl.Popup({ closeButton: false }).setHTML(
              `<strong>${SOURCE_LABELS.fema} (county-level declaration point — not a precise boundary)</strong><br/>${item.summary}`,
            ),
          )
          .addTo(map);
        markersRef.current.push(marker);
      ...
      // SVI is an aggregate reading across N tracts at one bbox centroid
      // (returnGeometry=false — see population_svi.py), never a real tract
      // shape. A blurred circle-layer halo, not a filled polygon, keeps that
      // honest: sized/colored for visibility, not to any real spatial extent.
      for (const item of items.filter((i) => i.source === "population_svi")) {
        ...
        const srcId = `svi-halo-${item.item_id}`;
        map.addSource(srcId, {
          type: "geojson",
          data: { type: "Feature", properties: {}, geometry: { type: "Point", coordinates: [item.longitude, item.latitude] } },
        });
        map.addLayer({
          id: `${srcId}-layer`,
          type: "circle",
          source: srcId,
          paint: { "circle-radius": 42, "circle-color": color, "circle-opacity": 0.22, "circle-blur": 1 },
        });
        const el = document.createElement("div");
        el.style.width = "10px";
        ...
        const marker = new maplibregl.Marker({ element: el })
          ...
          .setPopup(
            new maplibregl.Popup({ closeButton: false }).setHTML(
              `<strong>${SOURCE_LABELS.population_svi} (aggregate across ${raw.tract_count ?? "?"} tracts — centroid shown, not a tract boundary)</strong><br/>${item.summary}`,
            ),
          )
          .addTo(map);
        markersRef.current.push(marker);
